...
export function getAllModules() {
  ...
  typesFull.forEach(t => {
    ...
    try {
      const extraData = JSON.parse(t.extra_data!);
      const groupId = extraData.groupID;
      
      // Check if this is a charge/ammunition type 
      // (has chargeSize, is in known charge groups, or has damage attributes)
      const dogma = getDogmaAttributes(t.type_id, false);
      
      // Check various indicators that this is a charge/ammunition
      const isChargeGroup = CHARGE_GROUP_IDS.includes(groupId);
      const hasChargeSize = dogma.chargeSize !== undefined;
      const hasChargeDamage = (dogma as any).emDamage || (dogma as any).explosiveDamage || (dogma as any).kineticDamage || (dogma as any).thermalDamage;
      
      if (isChargeGroup || hasChargeSize || hasChargeDamage) {
        allCharges.push({
          typeId: t.type_id,
          typeName: getTypeName(t.type_id),
          groupId: groupId,
  ...
  const modules = typesFull
    ...
    .map(m => {
      ...
      // For high slot modules, always show charges as fallback
      // This ensures weapons without proper charge group data still show the charge button
      if ((compatibleCharges.length === 0 || slotType === 'high') && allCharges.length > 0) {
        // For high slot modules, add a broad selection of charges as fallback
        if (slotType === 'high') {
