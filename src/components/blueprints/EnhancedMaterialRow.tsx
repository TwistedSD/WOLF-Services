...
export const EnhancedMaterialRow: React.FC<EnhancedMaterialRowProps> = ({
  ...
  const isBaseMaterial =
    node.is_base_material ||
    (node.inputs.length === 0 && node.blueprint_id === null);
  const hasByproducts = node.byproducts && node.byproducts.length > 0;
  ...
  return (
    <div>
        ...
        {hasAlternatives &&
          ...
          blueprintOptions.length > 0 && (
            <select
              ...
              onChange={(e) =>
                onBlueprintChange(node.type_id, parseInt(e.target.value))
              }
            ...
            >
              {blueprintOptions.map((opt) => (
                <option key={opt.blueprint_id} value={opt.blueprint_id}>
                  {opt.facility_name || "Unknown"} - {opt.output_quantity} units
                  ({opt.time_seconds}s)
                </option>
              ))}
            </select>
      ...
      {showByproducts && hasByproducts && (
        ...
        >
          <div className="text-xs font-semibold text-foreground-muted mb-1">
            Byproducts:
          </div>
